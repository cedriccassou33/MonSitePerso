...
// ====== ANTI DOUBLE INIT (si app.js est chargé 2 fois) ======
if (window.__ACTIONS_APP_ALREADY_LOADED__) {
  // Si le script est évalué une 2e fois, on ne relance pas tout.
  console.warn("app.js déjà chargé — init ignoré.");
} else {
  window.__ACTIONS_APP_ALREADY_LOADED__ = true;
  ...
  // Client Supabase idempotent (ne casse pas si rechargé)
  ...
  async function testActionsTable() {
    ...
    try {
      ...
      if (error) {
        el.innerHTML = `
          Table <b>actions</b> : <span style="color:#b42318;">KO ❌</span>
  ...
  (async function init() {
    ...
    currentUser = user;
    const ui = $('#userInfo');
    if (ui) ui.textContent = user.email ?? user.id;
  ...
  async function loadActions() {
    ...
    if (error) {
      setStatus(`Erreur de chargement : ${escapeHtml(error.message)}`, false, 'list');
      return;
    }
  ...
  function applyFiltersAndRender() {
    const fEtat = $('#filterEtat')?.value ?? 'all';
    const fPrio = $('#filterPriorite')?.value ?? 'all';
    const sortField = $('#sortField')?.value ?? 'echeance';
    const asc = ($('#sortDir')?.value ?? 'asc') === 'asc';
  ...
  function bindUI() {
    ...
    $('#tbody')?.addEventListener('click', (e) => {
      ...
      else if (btn.classList.contains('btn-del')) onDelete(id);
    });
  }
  ...
  async function onCreate(e) {
    ...
    const { error } = await supabase.rpc('create_action', {
    ...
    if (error) {
      setStatus(`Erreur de création : ${escapeHtml(error.message)}`, false, 'create');
      return;
    }
  ...
  async function onSaveEdit(e) {
    ...
    if (error) {
      setStatus(`Erreur de mise à jour : ${escapeHtml(error.message)}`, false, 'edit');
      return;
    }
